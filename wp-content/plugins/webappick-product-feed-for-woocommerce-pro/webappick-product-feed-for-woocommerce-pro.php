...
if ( ! defined( 'WOO_FEED_PRO_VERSION' ) ) {
	...
	define( 'WOO_FEED_PRO_VERSION', '3.7.5' );
...
if ( ! defined( 'WOO_FEED_LIBS_PATH' ) ) {
	 ...
	 * @var string
	 */
	define( 'WOO_FEED_LIBS_PATH', WOO_FEED_PRO_PATH . 'libs/' );
}
if ( ! defined( 'WOO_FEED_PLUGIN_URL' ) ) {
	 ...
	 * @since 3.1.37
	 */
	define( 'WOO_FEED_PLUGIN_URL', trailingslashit( plugin_dir_url( WOO_FEED_PRO_PATH ) ) );
}
 ...
 */
require_once WOO_FEED_PRO_PATH . 'includes/hooks.php';
require_once WOO_FEED_PRO_PATH . 'includes/pro-hooks.php';
require_once WOO_FEED_PRO_PATH . 'includes/log-helper.php';
require_once WOO_FEED_PRO_PATH . 'includes/helper.php';
require_once WOO_FEED_PRO_PATH . 'includes/pro-helper.php';
require_once WOO_FEED_PRO_PATH . 'includes/cron-helper.php';

/**
 * Installer
 */
require_once WOO_FEED_PRO_PATH . 'includes/class-woo-feed-installer.php';
...
if ( ! class_exists( 'Woo_Feed' ) ) {
	 ...
	 */
	require WOO_FEED_PRO_PATH . 'includes/class-woo-feed.php';
}
...
if ( ! function_exists( 'run_woo_feed' ) ) {
	...
	function run_woo_feed() {
		$plugin = new Woo_Feed();
		register_activation_hook( WOO_FEED_PRO_FILE, [ 'Woo_Feed_Pro_Installer', 'install' ] );
	...
	function woo_feed_get_product_information() {
		...
		if ( woo_feed_wc_version_check( 3.2 ) ) {
			...
			try {
				...
				$products = new Woo_Feed_Products_v3_Pro( $getConfig );
...
if ( ! function_exists( 'woo_feed_generate_new_feed' ) ) {
	...
	function woo_feed_generate_new_feed() {
		if ( isset( $_POST['provider'], $_POST['_wpnonce'], $_POST['filename'], $_POST['feedType'] ) ) {
			...
			require WOO_FEED_PRO_ADMIN_PATH . 'partials/woo-feed-admin-display.php';
		}
	}
}
		...
		if ( isset( $_GET['action'] ) && 'edit-feed' == $_GET['action'] ) {
			...
			if ( isset( $_GET['feed'] ) && ! empty( $_GET['feed'] ) ) {
				...
				if ( false !== $feedInfo ) {
					...
					require WOO_FEED_PRO_ADMIN_PATH . 'partials/woo-feed-edit-template.php';
				} else {
			...
			require WOO_FEED_PRO_ADMIN_PATH . 'partials/woo-feed-manage-list.php';
		}
	}
}
...
if ( ! function_exists( 'feed_merchant_view' ) ) {
	...
	function feed_merchant_view() {
		...
		ob_start();
		require_once WOO_FEED_PRO_ADMIN_PATH . 'partials/woo-feed-edit-tabs.php';
		wp_send_json_success( [
			'tabs'         => ob_get_clean(),
...
// update_option( 'woo_feed_currency_api_code', $currencyAPI, false );
		...
		require WOO_FEED_PRO_ADMIN_PATH . 'partials/woo-feed-settings.php';
	}
}
...
// End of file webappick-product-feed-for-woocommerce-pro.php.
