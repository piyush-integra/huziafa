...
class Woo_Feed_Admin {
	...
	public function enqueue_styles( $hook ) {
		...
		if ( false !== strpos( $hook, 'webappick' ) && false !== strpos( $hook, 'feed' ) ) {
			...
			wp_enqueue_style( 'selectize', plugin_dir_url( __FILE__ ) . 'css/selectize' . $ext, array(), $this->version );
	...
	public function enqueue_scripts( $hook ) {
		...
		if ( false !== strpos( $hook, 'webappick' ) && false !== strpos( $hook, 'feed' ) ) {
			...
			if ( ! wp_script_is( 'clipboard', 'registered' ) ) {
				wp_register_script( 'clipboard', plugin_dir_url( __FILE__ ) . 'js/clipboard.min.js', array(), '2.0.4', false );
			}
			...
			wp_register_script( 'feed-pro', plugin_dir_url( __FILE__ ) . 'js/woo-feed-admin-pro' . $ext, [ $this->woo_feed ], $this->version, false );
...
//			if ( woo_feed_pro_is_activated() ) {
				...
				add_submenu_page( 'webappick-manage-feeds', __( 'Manage Attribute Mapping', 'woo-feed' ), __( 'Attributes Mapping', 'woo-feed' ), 'manage_woocommerce', 'webappick-manage-attribute-mapping', 'woo_feed_manage_attribute_mapping' );
				add_submenu_page( 'webappick-manage-feeds', __( 'Manage Dynamic Attributes', 'woo-feed' ), __( 'Dynamic Attributes', 'woo-feed' ), 'manage_woocommerce', 'webappick-manage-feed-attribute', 'woo_feed_manage_attribute' );
...
function woo_feed_pro_is_activated() {
	if ( apply_filters( 'WebAppick_is_local', in_array( $_SERVER['REMOTE_ADDR'], [ '127.0.0.1', '::1' ] ) ) ) {
		return true;
	}
