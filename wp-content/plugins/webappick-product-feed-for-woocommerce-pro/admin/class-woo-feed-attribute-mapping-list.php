 ...
 */
class Woo_Feed_Attribute_Mapping_List extends Woo_Feed_List_Table {
	...
	function column_default( $item, $column_name ) {
		...
		switch ( $column_name ) {
			...
			case 'provider':
				return sprintf( '<strong>%s</strong>', implode( $itemInfo['glue'], $itemInfo['mapping'] ) );
				break;
			case 'view':
				...
				return sprintf(
					'<a class="button button-primary" href="?page=%s&action=%s&mapping_name=%s&_wpnonce=%s">' . __( 'Edit', 'woo-feed' ) . '</a>',
	            ...
	            sprintf(
			        '<a val="?page=%s&action=%s&mapping_id=%s&_wpnonce=%s" class="button single-category-delete" style="cursor: pointer;">' . __( 'Delete', 'woo-feed' ) . '</a>',
			        esc_attr( $plugin_page ),
			        'delete-mapping',
	...
	function column_option_name( $item ) {
		...
		$delete_nonce = wp_create_nonce( 'wf_delete_mapping' );
		$actions = array(
			'edit'   => sprintf( '<a href="?page=%s&action=%s&mapping_name=%s&_wpnonce=%s">' . __( 'Edit',
					'woo-feed' ) . '</a>',
				esc_attr( $plugin_page ),
				'edit-mapping',
				$item['option_name'],
				$edit_nonce ),
			'delete' => sprintf( '<a val="?page=%s&action=%s&mapping_id=%s&_wpnonce=%s" class="single-category-delete" style="cursor: pointer;">' . __( 'Delete',
					'woo-feed' ) . '</a>',
				esc_attr( $plugin_page ),
				'delete-mapping',
				absint( $item['option_id'] ),
	...
	public static function get_mappings() {
		...
		$result = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $wpdb->options WHERE option_name LIKE %s;", 'wp_attr_mapping_%' ), 'ARRAY_A' );
	...
	public static function record_count() {
		global $wpdb;
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
		return $wpdb->get_var( $wpdb->prepare( "SELECT * FROM $wpdb->options WHERE option_name like %s", Woo_Feed_Products_v3_Pro::PRODUCT_ATTRIBUTE_MAPPING_PREFIX . '%' ) );
	}
	...
	function column_name( $item ) {
		...
		$actions      = array(
			'edit'   => sprintf( '<a href="?page=%s&action=%s&mapping_name=%s&_wpnonce=%s">' . __( 'Edit', 'woo-feed' ) . '</a>',
				esc_attr( $plugin_page ),
				'edit-mapping',
				absint( $item['option_id'] ),
				$edit_nonce ),
			'delete' => sprintf( '<a val="?page=%s&action=%s&mapping_id=%s&_wpnonce=%s" class="single-category-delete" style="cursor: pointer;">' . __( 'Delete', 'woo-feed' ) . '</a>',
				esc_attr( $plugin_page ),
				'delete-mapping',
				absint( $item['option_id'] ),
				$delete_nonce ),
	...
	function get_columns() {
		$columns = array(
			...
			'option_name' => __( 'Attribute Name', 'woo-feed' ),
			'provider'    => __( 'Preview', 'woo-feed' ),
	...
	public function process_bulk_action() {
		...
		if ( 'delete-mapping' === $this->current_action() ) {
			...
			if ( ! wp_verify_nonce( $nonce, 'wf_delete_mapping' ) ) {
				...
				wp_safe_redirect( admin_url( 'admin.php?page=webappick-manage-attribute-mapping&wpf_message=error' ) );
				die();
			} else {
				if ( isset( $_GET['mapping_id'] ) && self::delete_mapping( absint( $_GET['mapping_id'] ) ) ) {
					update_option( 'wpf_message', esc_html__( 'Mapping Deleted Successfully', 'woo-feed' ), false );
					wp_safe_redirect( admin_url( 'admin.php?page=webappick-manage-attribute-mapping&wpf_message=success' ) );
					die();
				} else {
