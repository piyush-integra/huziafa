...
class Woo_Feed_installer {
	...
	public static function check_version() {
		if ( ! defined( 'IFRAME_REQUEST' ) && version_compare( get_option( 'woo_feed_pro_version' ), WOO_FEED_PRO_VERSION, '<' ) ) {
	...
	/**
	 * Deactivate Free Version before all other actions
	 */
	private static function deactivate_free_version() {
		// Deactivate Free version.
		if ( woo_feed_is_plugin_active( 'webappick-product-feed-for-woocommerce/woo-feed.php' ) ) {
			wooFeed_deactivate_plugins( 'webappick-product-feed-for-woocommerce/woo-feed.php' );
		}
	}
	...
	private static function migrate_db() {
		...
		// settings api update.
		if ( version_compare( '3.3.10', WOO_FEED_PRO_VERSION, '<' ) ) {
			$keys = [ 'per_batch', 'product_query_type', 'currency_api_code', 'enable_error_debugging', 'woo_feed_cache_ttl' ];
			$data = [];
			foreach ( $keys as $key ) {
	...
	private static function update_woo_feed_version() {
		delete_option( 'woo_feed_pro_version' );
		delete_option( 'woo_feed_version' ); // old option.
		update_option( 'woo_feed_pro_version', WOO_FEED_PRO_VERSION, false );
		if ( ! defined( 'WOO_FEED_UPDATING' ) ) {
			update_option( 'woo-feed-pro-activation-time', time(), false );
		}
	}
}
