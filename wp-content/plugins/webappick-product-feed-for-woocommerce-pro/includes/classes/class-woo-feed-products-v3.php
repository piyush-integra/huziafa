...
class Woo_Feed_Products_v3 {
	...
	protected function get_query_args( $type = 'wc' ) {
		...
		if ( 'wc' === $type ) {
			$args = array(
				...
				'type'             => [ 'simple', 'variable', 'grouped', 'external','bundle', 'yith_bundle','subscription', 'variable-subscription'  ],
				'orderby'          => 'date',
				'order'            => 'DESC',
				'return'           => 'ids',
	...
	protected function process_xml_wrapper() {
		...
		if ( file_exists( WOO_FEED_PRO_ADMIN_PATH . 'partials/templates/' . $this->config['provider'] . '.txt' ) ) {
			$txt = file_get_contents( WOO_FEED_PRO_ADMIN_PATH . 'partials/templates/' . $this->config['provider'] . '.txt' );
			$txt = trim( $txt );
