	...
	class ReyCoreKirki {
		...
		public static function add_field( $config_id, $args ) {
			...
			if ( class_exists( 'Kirki' ) ) {
				...
				if( isset($args['settings']) && $setting = $args['settings']) {

					if( apply_filters("reycore/kirki_fields/disable_field={$setting}", false) ){

						do_action("reycore/kirki_fields/replace_field", $setting, $args);
						return;
					}

					$args = apply_filters("reycore/kirki_fields/field={$setting}", $args);
					$args = apply_filters("reycore/kirki_fields/field_type={$args['type']}", $args);
				}

				do_action("reycore/kirki_fields/before_field", $setting, $args);

				do_action("reycore/kirki_fields/before_field={$setting}", $args);

				Kirki::add_field( $config_id, $args );

				do_action("reycore/kirki_fields/after_field={$setting}", $args);
